...
static const uint8_t g_pageScrambleIdx[] = {
    (uint8_t)kParamScrambleAmt,(uint8_t)kParamScrambleRate,
    (uint8_t)kParamScrambleGlide,(uint8_t)kParamScramblePostCurve,
};
...
static void buildKinkModX(struct _CessnaAwg_DTC *d) {
    ...
    if (d->kinkGlide > 0.0001f) {
        // Take the shortest path around the wraparound boundary
        float diff = target - d->kinkPosSmoothed;
        if (diff >  (float)(kBins/2)) diff -= (float)kBins;
        if (diff < -(float)(kBins/2)) diff += (float)kBins;
        d->kinkPosSmoothed += d->kinkGlideSlewCoeff * diff;
        // Keep smoothed position in valid range
