/*
 * Cessna++ AWG (v34) – Phase-Segmented, Servo-Tracking Arbitrary Waveform Generator
 * --------------------------------------------------------------------------------
...
struct _CessnaAwg_DTC {
    ...
    float phaseCorrPerSample;  // correction applied each sample
    int   phaseCorrRemaining;  // samples remaining
...
// Shared correction: spread phase correction over kPhaseCorrWindow samples.
static inline void applySync(_CessnaAwg_DTC *d, float newPeriod) {
    ...
    // Phase correction — spread over window to avoid click.
    if (!d->hardJump && d->lockStrength > 0.0f) {
        // Total correction needed
        float totalCorr = d->phase * d->lockStrength;
        // Spread evenly over window (fold in any ongoing correction)
        float alreadyQueued = d->phaseCorrPerSample * (float)d->phaseCorrRemaining;
        d->phaseCorrRemaining  = _CessnaAwg_DTC::kPhaseCorrWindow;
        d->phaseCorrPerSample  = (totalCorr + alreadyQueued)
...
static void step(_NT_algorithm *base, float *busFrames, int numFramesBy4) {
    ...
    if (!in) {
        for (int n=0;n<numFrames;n++) {
            ...
            // Apply crossfade phase correction
    ...
    for (int n=0;n<numFrames;n++) {
        ...
        // Apply crossfade phase correction gradually
        if (d->phaseCorrRemaining > 0) {
            d->phase -= d->phaseCorrPerSample;
            d->phaseCorrRemaining--;
            if (d->phase < 0.0f)  d->phase += 1.0f;
