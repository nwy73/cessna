/*
 * Cessna++ AWG (v11) – Phase-Segmented, Servo-Tracking Arbitrary Waveform Generator
 * --------------------------------------------------------------------------------
...
static inline void computeModX(struct _CessnaAwg_DTC *d);
...
static void parameterChanged(_NT_algorithm *base, int p) {
    ...
    switch (p) {
        ...
        case kParamModMode:  d->modMode  = self->v[kParamModMode]; updateModGrayOut(base); break;
        case kParamModRate: {
...
static void finalizeCycle(struct _CessnaAwg_DTC *d, int cycleLen) {
    ...
    for (int i = 0; i < kBins; ++i)
        d->S[i] = d->A[i] + D * clampf(d->X[i] + d->modX[i], -1.0f, 1.0f);
    d->herm.computeSlopes(d->S);
...
static void step(_NT_algorithm *base, float *busFrames, int numFramesBy4) {
    ...
    for (int n=0;n<numFrames;n++) {
        ...
        if (edge) {
            ...
            if (d->modMode == 3) seqAdvance(d);  // sequencer clocked by detected cycle
...
static bool draw(_NT_algorithm *base) {
    ...
    // --- Parameter status line (top, replicating standard menu display) ---
    {
        int p = d->lastChangedParam;
        const _NT_parameter& param = g_parameters[p];
        ...
        if (param.unit == kNT_unitEnum && param.enumStrings) {
            // Enum: look up string
            int idx = val - param.min;
            if (idx >= 0 && param.enumStrings[idx])
                NT_drawText(W-2, 0, param.enumStrings[idx], 15, kNT_textRight, kNT_textNormal);
            else
                NT_drawText(W-2, 0, "?", 15, kNT_textRight, kNT_textNormal);
        } else {
            // Numeric: value + unit suffix
            int len = NT_intToString(valBuf, (int32_t)val);
            // Append unit suffix
            const char* suffix = "";
            switch (param.unit) {
                case kNT_unitPercent:  suffix = "%";  break;
                case kNT_unitHz:       suffix = "Hz"; break;
                case kNT_unitMs:       suffix = "ms"; break;
                case kNT_unitSeconds:  suffix = "s";  break;
                case kNT_unitDb:       suffix = "dB"; break;
                case kNT_unitSemitones:suffix = "st"; break;
                case kNT_unitCents:    suffix = "ct"; break;
                default: break;
            }
            // Append suffix in-place
            int s = 0;
            while (suffix[s]) valBuf[len++] = suffix[s++];
            valBuf[len] = '\0';
            NT_drawText(W-2, 0, valBuf, 15, kNT_textRight, kNT_textNormal);
        }
        // Parameter name on the left
        NT_drawText(2, 0, param.name, 15, kNT_textLeft, kNT_textNormal);
    }

    // Separator line below param line (text is 8px tall)
    NT_drawShapeI(kNT_line, 0, 9, W-1, 9, 3);
