...
struct _CessnaAwg_DTC {
    ...
    float kinkPostCurve;
    bool  kinkStepped;      // stepped readout for kink mode output
...
static const uint8_t g_pageKinkIdx[]     = {
    ...
    (uint8_t)kParamKinkGlide,(uint8_t)kParamKinkPostCurve,(uint8_t)kParamKinkStepped,
...
static void updateDisplay(struct _CessnaAwg_DTC *d) {
    ...
    bool useStep = d->stepped || (d->modMode == 2 && d->kinkStepped);
...
static void step(_NT_algorithm *base, float *busFrames, int numFramesBy4) {
    ...
    for (int n=0;n<numFrames;n++) {
        ...
        // Determine if stepped readout applies
        bool useStep = d->stepped || (d->modMode == 2 && d->kinkStepped);

        if (!useStep && postCurve > 0.0001f)
