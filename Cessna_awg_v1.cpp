...
static void buildKinkModX(struct _CessnaAwg_DTC *d) {
    // Apply glide: slew kinkPosSmoothed toward target, scaled to step period
    float target = (float)(((int)roundf(d->kinkPos) + d->kinkRangeStart) % kBins);
    if (d->kinkGlide > 0.0001f) {
        // At Glide=100% the position takes ~1 full step period to arrive
        float glideTime = d->kinkGlide * (float)d->kinkSamplePeriod;
        float slew = 1.0f - powf(0.001f, 1.0f / (glideTime + 1.0f));
        d->kinkPosSmoothed += slew * (target - d->kinkPosSmoothed);
    } else {
        d->kinkPosSmoothed = target;
    }

    int centrePos = (int)roundf(d->kinkPosSmoothed) % kBins;
