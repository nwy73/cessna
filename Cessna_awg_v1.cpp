/*
 * Cessna++ AWG (v31) – Phase-Segmented, Servo-Tracking Arbitrary Waveform Generator
 * --------------------------------------------------------------------------------
...
static void parameterChanged(_NT_algorithm *base, int p) {
    ...
    switch (p) {
        ...
        case kParamLock: {
            ...
            // Derive PLL bandwidth from lock strength:
            // Lock 100% → wide bandwidth (fast tracking); Lock 0% → narrow (slow)
            d->pllBandwidth = 0.005f + ls * 0.095f;  // 0.005..0.1
            break;
        }
    ...
    for (int n=0;n<numFrames;n++) {
        ...
        if (edge) {
            ...
            if (d->syncMode == 0) {
                ...
                // N-before-M: feed one observation; phase correction fires
                // only when N consistent lead or lag votes accumulate.
                // "lead" = phase > 0.5 (oscillator behind input), "lag" = phase < 0.5
