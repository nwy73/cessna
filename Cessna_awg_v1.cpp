/*
 * Cessna++ AWG (v39) – Phase-Segmented, Servo-Tracking Arbitrary Waveform Generator
 * --------------------------------------------------------------------------------
...
static constexpr float kEnvBeta        = 0.01f;    // envelope smoothing (per sample)
static constexpr float kMinEnvGate     = 0.005f;   // below this env level, suppress edge detection (silence gate)
...
// Simple one-pole low-pass filter for detection path
struct OnePoleLP {
    float z = 0.0f;
    float a = 1.0f; // 1.0 = bypass, lower = more filtering

    // Set coefficient from cutoff freq (Hz) and sample rate
    inline void setCutoff(float hz, float sr) {
        float w = 2.0f * 3.14159265f * hz / sr;
        a = w / (w + 1.0f);
    }
...
struct _CessnaAwg_DTC {
    ...
    float inputGain;        // 0.5x..10x applied before capture and detection
    OnePoleLP detLpf;       // LPF on detection path only
    float detLpfFreq;       // cutoff in Hz (0 = bypass)
    ...
    // n:m sync ratio — numerator/denominator (e.g. 1:2 = half speed, 2:1 = double)
    // Lookup table indexed by kParamSyncRatio value
    int   syncRatioP;   // numerator   (output periods)
    int   syncRatioQ;   // denominator (input periods)
    int   syncRatioCycleCount; // counts input edges within the Q-cycle group

    // Gradient frequency update accumulator
    float gradAccum;    // accumulates input * waveform_slope each sample
    int   gradSamples;  // sample count for normalisation
    ...
    float modAmt;           // 0..1 shared depth across all modes
    float rippleRate;       // Hz
    ...
    // Drunk mode
    float drunkWander;      // 0..1 step size per cycle
    float drunkValues[kBins]; // current bin values (-1..1)
    uint32_t drunkRng;      // separate RNG state for drunk

    // Combined modulation output
    ...
    int   algorithmIdx;     // cached from NT_algorithmIndex(), valid after first step()
    ...
    _CessnaAwg_DTC() {
        ...
        algorithmIdx = -1; // invalid until first step()
...
enum {
    ...
    // Mod page
    kParamModMode,      // 0=Off 1=Ripple 2=Sequencer 3=Drunk
    kParamModAmt,       // 0..100% shared
    kParamRippleRate,   // 1..1000 → 0.01..10 Hz
    kParamSeqSpeed,     // 1..32, inverted: higher param = faster (seqSpeed = 33 - param)
    kParamSeqSmooth,    // 0..100% slew
    kParamSeqDrift,     // 0..100% random walk
    kParamSeqPattern,   // 0=Ramp 1=Triangle 2=Random 3=Alternating
    kParamDrunkWander,  // 0..100% step size per cycle
    kParamSyncRatio,    // 0..7 enum: 1:1 1:2 2:1 1:3 3:1 2:3 3:2 3:4
    kParamInputGain,    // 50..1000 → 0.5x..10x input gain
    kParamDetLpf,       // 0..100: 0=bypass, 100=~500Hz cutoff on detection path only
};
...
static const char* syncRatioStrings[]= {"1:1", "1:2", "2:1", "1:3", "3:1", "2:3", "3:2", "3:4", nullptr};
static constexpr int kNumParams = (kParamDetLpf + 1);
...
static const _NT_parameter g_parameters[kNumParams] = {
    ...
    { .name="Stickiness",  .min=0,   .max=100, .def=0,   .unit=kNT_unitPercent, .scaling=kNT_scalingNone, .enumStrings=nullptr },
    ...
    { .name="Phase Lock",  .min=0,   .max=100, .def=100, .unit=kNT_unitPercent, .scaling=kNT_scalingNone, .enumStrings=nullptr },
    { .name="Pitch Track", .min=0,   .max=100, .def=100, .unit=kNT_unitPercent, .scaling=kNT_scalingNone, .enumStrings=nullptr },
    ...
    { .name="Sync Ratio",  .min=0,.max=7,   .def=0,  .unit=kNT_unitEnum,    .scaling=kNT_scalingNone, .enumStrings=syncRatioStrings },
    { .name="In Gain",   .min=50,  .max=1000,.def=100, .unit=kNT_unitNone,    .scaling=kNT_scalingNone, .enumStrings=nullptr },
    { .name="Det LPF",   .min=0,   .max=100, .def=0,   .unit=kNT_unitPercent, .scaling=kNT_scalingNone, .enumStrings=nullptr },
};
...
static const uint8_t g_pageMainIdx[] = {
    (uint8_t)kParamInputGain,
    (uint8_t)kParamDetLpf,
    (uint8_t)kParamAdapt,
    (uint8_t)kParamCurve,
    (uint8_t)kParamSyncMode,
    (uint8_t)kParamSyncRatio,
    (uint8_t)kParamLock,
    (uint8_t)kParamTrack,
    (uint8_t)kParamPllBw,
...
static _NT_algorithm* construct(const _NT_algorithmMemoryPtrs& ptrs,
    ...
    if (d->minPeriodSamps > 32)   d->minPeriodSamps = 32;
    if (d->maxPeriodSamps < 256)  d->maxPeriodSamps = 256;
    if (d->maxPeriodSamps > kMaxCycleSamps) d->maxPeriodSamps = kMaxCycleSamps;

    // Fix: derive lastPeriodSamples from actual sample rate (not default 48k)
    d->lastPeriodSamples  = (int)(sr / 110.0f);
...
static void updateModGrayOut(_NT_algorithm* base, int modeOverride = -1) {
    ...
    // Use explicit override when called from parameterChanged (self->v and d->modMode
    // may both lag since NT writes self->v after the callback). Fall back to d->modMode
    // for deferred calls from step() where DTC is always current.
    int mode     = (modeOverride >= 0) ? modeOverride : d->modMode;
    int syncMode = d->syncMode;
...
static void parameterChanged(_NT_algorithm *base, int p) {
    ...
    switch (p) {
        ...
        case kParamAdapt:   d->adaptAlpha   = 1.0f - getPct01(kParamAdapt); break;
        ...
        case kParamInputGain: {
            // 50..1000 → 0.5x..10x
            d->inputGain = (float)self->v[kParamInputGain] * 0.01f;
            break;
        }
        case kParamDetLpf: {
            // 0 = bypass (set cutoff very high), 100 = ~500Hz cutoff
            float pct = (float)self->v[kParamDetLpf] * 0.01f;
            if (pct < 0.005f) {
                d->detLpf.a = 1.0f; // bypass
            } else {
                // Map 0..1 → 20000Hz..500Hz (higher pct = more filtering = lower cutoff)
                float hz = 20000.0f * (1.0f - pct) + 500.0f * pct;
                d->detLpf.setCutoff(hz, d->sampleRate);
            }
            break;
        }
        ...
        case kParamSyncRatio: {
            // Lookup table: {P, Q} pairs for each enum option
            static const int ratioTable[8][2] = {
                {1,1}, {1,2}, {2,1}, {1,3}, {3,1}, {2,3}, {3,2}, {3,4}
            };
            int idx2 = self->v[kParamSyncRatio];
            if (idx2 < 0) idx2 = 0;
            if (idx2 > 7) idx2 = 7;
            d->syncRatioP = ratioTable[idx2][0];
            d->syncRatioQ = ratioTable[idx2][1];
            d->syncRatioCycleCount = 0;
            break;
        }
        default:
    ...
    // Always defer grayout to step() — self->v is updated by NT *after*
    // parameterChanged returns, so reading it here gives stale values.
    d->needsGrayOutUpdate = true;
}
 ...
 */
static bool processCycleDetection(struct _CessnaAwg_DTC *d, float x) {
    // DC-block for both envelope estimation AND edge detection.
    // Using the same signal for both ensures threshold and crossing tests
    // are consistent even when input has DC offset or asymmetry.
    float xDet = d->dc.process(x);
    float ay = fabsf(xDet);
    ...
    // Adaptive threshold: 10% of envelope, hard floor at 0.01 (normalised).
    const float thresh = fmaxf(0.01f, 0.10f * d->env);
    d->thresh = thresh;

    // Silence gate: if signal is too quiet, suppress edge detection entirely.
    // This prevents random edges from noise/silence churning period estimates.
    if (d->env < kMinEnvGate) {
        d->wasBelowNeg = false;
        return false;
    }
    ...
    // Arm when detection signal goes clearly negative.
    if (xDet < -thresh) d->wasBelowNeg = true;

    // Fire when: armed, detection signal clearly positive, AND hold-off elapsed.
    if (d->wasBelowNeg && (xDet > thresh) && (d->samplesSinceEdge >= d->minPeriodSamps)) {
        ...
        if (period <= d->maxPeriodSamps) {
            ...
            if (rel > 0.10f) {
                // Pitch jump — use fast beta instead of instant snap to avoid
                // sharp derivative discontinuity in output waveform
                const float fastBeta = 0.35f;
                d->smoothPeriod = (1.0f - fastBeta) * d->smoothPeriod
                                + fastBeta * (float)period;
                d->stableCount  = 0;
                d->locked       = false;
                d->hardJump     = true;
            } else {
...
static __attribute__((noinline)) void combineS(struct _CessnaAwg_DTC *d) {
    float D = d->depthD * 5.0f;  // offset depth: scales X[] only
    for (int i = 0; i < kBins; ++i) {
        // Offsets and mod are now independent:
        // D scales static bin offsets (X[]), modX[] is already scaled by modAmt
        float offset = clampf(D * clampf(d->X[i], -1.0f, 1.0f)
                     + d->modX[i] * 5.0f, -5.0f, 5.0f);
...
static void finalizeCycle(struct _CessnaAwg_DTC *d, int cycleLen) {
    ...
    for (int i = 0; i < kBins; ++i) {
        ...
        d->newA[i] = clampf(d->newA[i], -5.0f, 5.0f);
    }
    ...
    if (!d->freeze) {
        ...
        for (int i = 0; i < kBins; ++i) {
            ...
            d->A[i]  = clampf(d->A[i], -5.0f, 5.0f);  // prevent rail creep
        }
    }
...
// Called once per detected cycle to advance drunk random walk.
// When wander is at minimum, values decay toward zero so the effect
// disengages cleanly rather than freezing at last non-zero position.
static void drunkAdvance(struct _CessnaAwg_DTC *d) {
    if (d->drunkWander < 0.001f) {
        for (int i=0;i<kBins;i++)
            d->drunkValues[i] *= 0.9f;  // fast decay to zero (~16ms half-life at 440Hz)
        return;
    }
    for (int i=0;i<kBins;i++) {
...
// Compute modX[] for one sample.
// Ripple: per-sample LFO with full spread (fixed at 100%).
// Sequencer: slew seqCurrent toward seqTarget each sample.
// Drunk: updated per-cycle in drunkAdvance(), just outputs drunkValues here.
static __attribute__((noinline)) void computeModX(struct _CessnaAwg_DTC *d) {
    ...
    if (d->modMode == 1) {
        // Ripple: advance LFO, full spread (phase offset = 2pi * i/kBins)
        d->lfoPhase += d->lfoPhaseInc;
        if (d->lfoPhase >= 1.0f) d->lfoPhase -= 1.0f;
        float lfoRad = d->lfoPhase * 2.0f * M_PI_F;
        for (int i=0;i<kBins;i++) {
            float binPhase = lfoRad + 2.0f * M_PI_F * ((float)i / (float)kBins);
            d->modX[i] = d->modAmt * sinf(binPhase);
        }
    } else if (d->modMode == 2) {
        // Sequencer: slew seqCurrent toward seqTarget
        ...
        // Drunk: drunkValues updated per-cycle in drunkAdvance()
        for (int i=0;i<kBins;i++) {
...
static inline void applySync(_CessnaAwg_DTC *d, float newPeriod) {
    // Phase correction — simple unweighted nudge toward zero.
    // (Weighted correction by waveform slope was tried in v39 but caused
    // clicking due to stale S[] at edge moment — reverted.)
    if (!d->hardJump) {
        d->phase *= (1.0f - d->lockStrength);
    }
    // Frequency tracking with n:m ratio scaling
    if (d->trackStrength > 0.0f) {
        float ratio  = (float)d->syncRatioP / (float)d->syncRatioQ;
        float target = ratio / newPeriod;
...
// PLL: called each time an edge is detected. Updates frequency estimate
// continuously via a simple first-order loop filter. Phase correction
// still fires but is driven by the smoothed phase error.
static void pllUpdate(_CessnaAwg_DTC *d, float newPeriod) {
    // Phase error: difference between current phase and ideal (0 at edge)
    // In a PLL the phase error at the edge moment is just d->phase (should be ~0 or ~1)
    float phErr = d->phase;
    // Wrap to -0.5..0.5
    ...
    d->pllIntegrator += d->pllBandwidth * 0.1f * phErr;
    float correction  = d->pllBandwidth * phErr + d->pllIntegrator;
    clampf(d->pllIntegrator, -0.1f, 0.1f);

    // Update frequency estimate with n:m ratio scaling
    float ratio  = (float)d->syncRatioP / (float)d->syncRatioQ;
    float target = (ratio / newPeriod) * (1.0f - correction);
    if (d->trackStrength > 0.0f) {
        d->phaseIncTarget = d->phaseIncTarget
                          + d->trackStrength * (target - d->phaseIncTarget);
    }

    // Gentle phase nudge proportional to phase error (not a hard snap)
    d->phase -= d->lockStrength * phErr * 0.5f;
...
static void step(_NT_algorithm *base, float *busFrames, int numFramesBy4) {
    ...
    // Cache algorithm index — only valid from step() onwards
    if (d->algorithmIdx < 0) {
    ...
    if (d->needsGrayOutUpdate) {
        ...
        // By the time step() runs, self->v is fully updated by the NT.
        // Re-sync DTC fields that affect grayout from self->v.
        d->modMode  = self->v[kParamModMode];
        d->syncMode = self->v[kParamSyncMode];
    ...
    for (int n=0;n<numFrames;n++) {
        // Apply input gain with tanh soft clip — scales and saturates before
        // capture and detection. Unity gain (1.0x) at default (100).
        float xRaw = in[n];
        float xGained = xRaw * d->inputGain;
        // tanh soft clip: graceful saturation, no hard edges
        float x = tanhf(xGained);

        // Detection path: apply LPF to reduce transient noise / high-frequency content.
        // This only affects sync detection — waveform capture uses the gained signal x.
        float xDet = (d->detLpf.a < 0.9999f) ? d->detLpf.process(x) : x;

        if (d->cycleWrite < kMaxCycleSamps) {
            d->cycleBuf[d->cycleWrite++] = x;  // capture gained (not LPF'd) signal
        }
        ...
        bool edge = processCycleDetection(d, xDet);
        if (edge) {
...
static bool draw(_NT_algorithm *base) {
    ...
    for (int n=1;n<kDisplayPoints;n++) {
        ...
        NT_drawShapeI(kNT_line, x0, yOut0, x1, yOut1, 15); // second pass = bold
    }
