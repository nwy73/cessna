...
static void updateDisplay(struct _CessnaAwg_DTC *d) {
    ...
    // Pre-compute slopes if needed
    if (!useStep && d->preCurve > 0.0001f) d->preHerm.computeSlopes(d->baseRaw);
    if (!useStep && postCurve   > 0.0001f) d->herm.computeSlopes(d->S);
    ...
    for (int k=0;k<kDisplayPoints;k++) {
        ...
        if (useStep) {
            ...
            float modVal   = evalLinear(d->modX, ph) * 5.0f;
...
static void step(_NT_algorithm *base, float *busFrames, int numFramesBy4) {
    ...
    for (int n=0;n<numFrames;n++) {
        ...
        // Pre-smooth: compute slopes of baseRaw for Hermite evaluation
        if (!useStep && d->preCurve > 0.0001f)
            d->preHerm.computeSlopes(d->baseRaw);
        // Post-smooth: compute slopes of S[]
        ...
        if (useStep) {
            ...
            // Mod contribution (linear interpolation of modX)
            float modVal = evalLinear(d->modX, d->phase) * 5.0f;
            ...
            // Post-smooth: blend S[] linear and Hermite
