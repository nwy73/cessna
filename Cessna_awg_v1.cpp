...
struct _CessnaAwg_DTC {
    ...
    float depthD;
    float preCurve;          // 0..1 Hermite blend on B[] before modulation
    float curve;             // retained for Stepped grayout — not used for audio
    bool  stepped;
    ...
    PeriodicHermite16 preHerm;   // pre-smooth hermite (on B[])
    float smoothedB[kBins];      // pre-smoothed B[], used as base in combineS
    ...
    _CessnaAwg_DTC() {
        ...
        for (int i=0;i<kBins;i++) smoothedB[i] = 0.0f;
        phase   = 0.0f;
...
enum {
    ...
    // Main page
    kParamDepth,
    kParamPreCurve,     // pre-modulation smoothing
    kParamStepped,
...
static const uint8_t g_pageMainIdx[] = {
    (uint8_t)kParamDepth,(uint8_t)kParamPreCurve,(uint8_t)kParamStepped,
};
...
static void combineS(struct _CessnaAwg_DTC *d) {
    ...
    // Apply pre-smooth: blend raw toward Hermite-smoothed version
    if (d->preCurve > 0.0001f) {
        d->preHerm.computeSlopes(raw);
        for (int i=0;i<kBins;i++) {
            float ph = ((float)i + 0.5f) / (float)kBins;
            float smoothed = d->preHerm.eval(raw, ph);
            d->smoothedB[i] = lerpf(raw[i], smoothed, d->preCurve);
        }
    } else {
        for (int i=0;i<kBins;i++) d->smoothedB[i] = raw[i];
    }

    // Combine smoothed base + modX → S[]
    float D = d->depthD;
    for (int i=0;i<kBins;i++)
        d->S[i] = D * tanhf((d->smoothedB[i] + d->modX[i] * 5.0f) / 5.0f) * 5.0f;
}
...
static void buildKinkModX(struct _CessnaAwg_DTC *d) {
    // Apply glide: slew kinkPosSmoothed toward target kinkPos
    float target = (float)(((int)roundf(d->kinkPos) + d->kinkRangeStart) % kBins);
    if (d->kinkGlide > 0.0001f) {
        float slew = 1.0f - d->kinkGlide * 0.999f;
...
static void updateDisplay(struct _CessnaAwg_DTC *d) {
    float postCurve = 0.0f;
    if      (d->modMode == 2) postCurve = d->kinkPostCurve;
    else if (d->modMode == 3) postCurve = d->scramblePostCurve;
...
static void step(_NT_algorithm *base, float *busFrames, int numFramesBy4) {
    ...
    for (int n=0;n<numFrames;n++) {
        ...
        // Determine post-smooth for this mode
        float postCurve = 0.0f;
        if      (d->modMode == 2) postCurve = d->kinkPostCurve;
        else if (d->modMode == 3) postCurve = d->scramblePostCurve;
