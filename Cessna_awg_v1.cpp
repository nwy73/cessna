...
struct _CessnaAwg_DTC {
    ...
    float curve;             // 0..1 Hermite blend on S[] after modulation (final output)
    bool  stepped;
    ...
    PeriodicHermite16 baseHerm;  // pre-smooth hermite (on raw base bins)

    // Per-mode post-smooth
    float kinkPostCurve;
    float scramblePostCurve;

    // Kink glide: slews kink position between steps
    float kinkGlide;             // 0..1 slew coefficient (0=instant, 1=never arrives)
    float kinkGlideSlewCoeff;    // cached per-sample slew — recomputed when Rate or Glide changes
    ...
    int   kinkDirection; // 0=Fwd 1=Back 2=Alt 3=Random 4=Drunk
    int   kinkSampleCount;
    int   kinkSamplePeriod;

    // Wave position state
    float kinkPos;       // current centre position (float, 0..kBins-1)
    float kinkDrunkPos;  // for Drunk direction: current pos as float
    float kinkDrunkStep; // max step per advance for Drunk
    ...
    void updateKinkGlideSlewCoeff() {
        if (kinkGlide < 0.0001f) {
            kinkGlideSlewCoeff = 1.0f; // instant
        } else {
...
enum {
    ...
    kParamCurve,        // post-modulation interpolation blend
    kParamStepped,
...
static const char* kinkDirStrings[]  = {"Forward","Backward","Alternating","Random","Drunk",nullptr};
...
static const _NT_parameter g_parameters[kNumParams] = {
    ...
    { .name="Curve",     .min=0,  .max=100,.def=100,.unit=kNT_unitPercent,.scaling=kNT_scalingNone,.enumStrings=nullptr },
    ...
    { .name="Direction",.min=0,.max=4,  .def=0, .unit=kNT_unitEnum,  .scaling=kNT_scalingNone,.enumStrings=kinkDirStrings },
    { .name="Glide",    .min=0,.max=100,.def=0, .unit=kNT_unitPercent,.scaling=kNT_scalingNone,.enumStrings=nullptr },
    { .name="Post-smooth",.min=0,.max=100,.def=0,.unit=kNT_unitPercent,.scaling=kNT_scalingNone,.enumStrings=nullptr },
...
static const uint8_t g_pageMainIdx[] = {
    (uint8_t)kParamDepth,(uint8_t)kParamPreCurve,(uint8_t)kParamCurve,(uint8_t)kParamStepped,
};
...
static const uint8_t g_pageKinkIdx[]     = {
    ...
    (uint8_t)kParamKinkGlide,(uint8_t)kParamKinkPostCurve,
};
static const uint8_t g_pageScrambleIdx[] = {
    (uint8_t)kParamScrambleAmt,(uint8_t)kParamScrambleRate,(uint8_t)kParamScramblePostCurve,
};
...
static void combineS(struct _CessnaAwg_DTC *d) {
    ...
    // Build raw base (morph between slots)
    float raw[kBins];
    for (int i=0;i<kBins;i++)
        raw[i] = lerpf(d->slotA[i], d->slotB[i], d->morph);

    // Pre-smooth: evaluate Hermite at each bin centre, blend with raw
    float base[kBins];
    if (d->preCurve > 0.0001f) {
        d->baseHerm.computeSlopes(raw);
        for (int i=0;i<kBins;i++) {
            float ph = ((float)i + 0.5f) / (float)kBins;
            float hermVal = d->baseHerm.eval(raw, ph);
            base[i] = lerpf(raw[i], hermVal, d->preCurve);
        }
    } else {
        for (int i=0;i<kBins;i++) base[i] = raw[i];
    }

    // Combine pre-smoothed base + modX → S[], with tanh soft clip
    float D = d->depthD;
    for (int i=0;i<kBins;i++)
        d->S[i] = D * tanhf((base[i] + d->modX[i] * 5.0f) / 5.0f) * 5.0f;
}
...
static void advanceKinkPosition(struct _CessnaAwg_DTC *d) {
    ...
    switch (d->kinkDirection) {
        ...
        case 4: // Drunk — random walk within range
            d->kinkDrunkPos += lcgRand(d->scrambleRng) * d->kinkDrunkStep;
            d->kinkDrunkPos  = clampf(d->kinkDrunkPos, 0.0f, range);
            d->kinkPos       = d->kinkDrunkPos;
            break;
    }
}
...
static void buildKinkModX(struct _CessnaAwg_DTC *d) {
    // Apply glide using cached slew coefficient
    float target = (float)(((int)roundf(d->kinkPos) + d->kinkRangeStart + kBins * 4) % kBins);
    if (d->kinkGlide > 0.0001f) {
        d->kinkPosSmoothed += d->kinkGlideSlewCoeff * (target - d->kinkPosSmoothed);
    } else {
        d->kinkPosSmoothed = target;
    }

    // Ensure centrePos is always valid positive index
    int centrePos = ((int)roundf(d->kinkPosSmoothed) % kBins + kBins) % kBins;
    int halfWidth = d->kinkWidth / 2;
    if (halfWidth < 1) halfWidth = 1;

    for (int i=0;i<kBins;i++) {
        float env = kinkEnvelope(d->kinkShape, i, centrePos, halfWidth);
        d->modX[i] = d->kinkAmt * env;
    }
}
...
static void parameterChanged(_NT_algorithm *base, int p) {
    ...
    switch (p) {
        ...
        case kParamKinkRange:
            ...
            if (d->kinkDrunkPos >= (float)d->kinkRange) d->kinkDrunkPos = 0.0f;
            break;
        case kParamKinkDirection:
            d->kinkDirection = self->v[kParamKinkDirection];
            d->kinkDrunkStep = 1.0f;
            break;
        case kParamKinkGlide:
            d->kinkGlide = getPct01(kParamKinkGlide);
            d->updateKinkGlideSlewCoeff();
            break;
        case kParamKinkPostCurve:
            d->kinkPostCurve = getPct01(kParamKinkPostCurve);
            break;
        ...
        case kParamScramblePostCurve:
            d->scramblePostCurve = getPct01(kParamScramblePostCurve);
            break;
...
static void updateDisplay(struct _CessnaAwg_DTC *d) {
    float postCurve = d->curve;
    if      (d->modMode == 2 && d->kinkPostCurve    > postCurve) postCurve = d->kinkPostCurve;
    else if (d->modMode == 3 && d->scramblePostCurve > postCurve) postCurve = d->scramblePostCurve;
    ...
    for (int k=0;k<kDisplayPoints;k++) {
        ...
        if (d->stepped) {
            ...
            float cur = (postCurve > 0.0001f) ? d->herm.eval(d->S, ph) : lin;
            y = lerpf(lin, cur, postCurve);
        }
        d->dispWave[k] = y;
    }
}
...
static void step(_NT_algorithm *base, float *busFrames, int numFramesBy4) {
    ...
    for (int n=0;n<numFrames;n++) {
        ...
        // Determine post-smooth: per-mode or global curve
        float postCurve = d->curve;
        if      (d->modMode == 2 && d->kinkPostCurve    > postCurve) postCurve = d->kinkPostCurve;
        else if (d->modMode == 3 && d->scramblePostCurve > postCurve) postCurve = d->scramblePostCurve;

        if (!d->stepped && postCurve > 0.0001f)
        ...
        if (d->stepped) {
            ...
            float cur = (postCurve > 0.0001f) ? d->herm.eval(d->S, d->phase) : lin;
            y = lerpf(lin, cur, postCurve);
        }
