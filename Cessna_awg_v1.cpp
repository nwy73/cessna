/*
 * Cessna++ AWG (v33) – Phase-Segmented, Servo-Tracking Arbitrary Waveform Generator
 * --------------------------------------------------------------------------------
...
struct _CessnaAwg_DTC {
    ...
    // Crossfade phase correction (Edge + N-before-M modes)
    float phaseCorrPerSample;   // phase correction applied each sample
    int   phaseCorrRemaining;   // samples of correction left to apply
    static constexpr int kPhaseCorrWindow = 128; // ~2.7ms at 48kHz

    // PLL per-sample state
    float pllFreqEstimate;  // base frequency estimate (cycles/sample)
    float pllIntegrator;    // PI loop integrator
    float pllPhaseErr;      // last measured phase error (for display/debug)
    float pllBandwidth;     // loop bandwidth coefficient (set via kParamPllBw)
    ...
    _CessnaAwg_DTC() {
        ...
        pllFreqEstimate = 1.0f / smoothPeriod;
        pllIntegrator   = 0.0f;
        pllPhaseErr     = 0.0f;
        pllBandwidth    = 0.005f;
        ...
        phaseCorrPerSample  = 0.0f;
        phaseCorrRemaining  = 0;

        pllFreqEstimate = phaseInc;
        pllIntegrator   = 0.0f;
        pllPhaseErr     = 0.0f;
        pllBandwidth    = 0.005f;
...
static _NT_algorithm* construct(const _NT_algorithmMemoryPtrs& ptrs,
    ...
    d->pllFreqEstimate = d->phaseInc;
    // Slew: ~5ms time constant.
...
// Shared frequency update — called by Edge and N-before-M on correction events.
// Phase correction is now spread over kPhaseCorrWindow samples (crossfade).
static inline void applySync(_CessnaAwg_DTC *d, float newPeriod) {
    ...
    // Phase correction — spread over window rather than instant snap.
    // Total correction = phase * lockStrength (same amount as before, but gradual).
    if (!d->hardJump && d->lockStrength > 0.0f) {
        float totalCorr = d->phase * d->lockStrength;
        // Add to any ongoing correction (in case of rapid edges)
        d->phaseCorrPerSample  = (totalCorr + d->phaseCorrPerSample * (float)d->phaseCorrRemaining)
                                 / (float)_CessnaAwg_DTC::kPhaseCorrWindow;
        d->phaseCorrRemaining  = _CessnaAwg_DTC::kPhaseCorrWindow;
    }
}

// N-before-M vote feed. Returns true if a correction fired.
...
// PLL per-sample update — called every sample in PLL mode.
// Uses phase error to continuously adjust phaseIncTarget.
static inline void pllPerSample(_CessnaAwg_DTC *d) {
    // Phase error: current phase should be approaching 0 at each edge.
    // Between edges, we track the error as phase drift from expected.
    // Wrap to -0.5..0.5
    float phErr = d->phase;
    ...
    if (d->pllIntegrator >  0.1f) d->pllIntegrator =  0.1f;
    if (d->pllIntegrator < -0.1f) d->pllIntegrator = -0.1f;

    float correction = d->pllBandwidth * phErr + d->pllIntegrator;

    // Nudge frequency estimate
    d->pllFreqEstimate = d->pllFreqEstimate * (1.0f - correction);
    // Clamp to valid range
    float minInc = 1.0f / (float)d->maxPeriodSamps;
    float maxInc = 1.0f / (float)d->minPeriodSamps;
    if (d->pllFreqEstimate < minInc) d->pllFreqEstimate = minInc;
    if (d->pllFreqEstimate > maxInc) d->pllFreqEstimate = maxInc;

    // Apply track strength
    if (d->trackStrength > 0.0f) {
        d->phaseIncTarget = d->phaseIncTarget
                          + d->trackStrength * (d->pllFreqEstimate - d->phaseIncTarget);
    }
}

// PLL edge event — update base frequency estimate from measured period.
static inline void pllOnEdge(_CessnaAwg_DTC *d, float newPeriod) {
    // Update base frequency estimate from measured period
    float measuredFreq = 1.0f / newPeriod;
    // Blend measured into estimate with trackStrength
    if (d->trackStrength > 0.0f) {
        d->pllFreqEstimate = d->pllFreqEstimate
                           + d->trackStrength * 0.3f * (measuredFreq - d->pllFreqEstimate);
    }
    // Reset integrator on hard jump to avoid windup
...
static void step(_NT_algorithm *base, float *busFrames, int numFramesBy4) {
    ...
    if (!in) {
        for (int n=0;n<numFrames;n++) {
            ...
            // PLL per-sample (no input — runs open loop on existing estimate)
            if (d->syncMode == 2) pllPerSample(d);

            // Crossfade phase correction
            if (d->phaseCorrRemaining > 0) {
                d->phase -= d->phaseCorrPerSample;
                d->phaseCorrRemaining--;
    ...
    for (int n=0;n<numFrames;n++) {
        ...
        if (edge) {
            ...
            if (d->syncMode == 0) {
                // Edge mode: crossfade phase correction on every detected edge
                applySync(d, newPeriod);
            } else if (d->syncMode == 1) {
                // N-before-M: pre-condition with Edge until locked
                if (!d->locked) {
                    applySync(d, newPeriod);
                } else {
                ...
                // PLL: update base frequency estimate on edge
                pllOnEdge(d, newPeriod);
            }
        ...
        // PLL per-sample frequency correction
        if (d->syncMode == 2) {
            pllPerSample(d);
        }

        // Apply crossfade phase correction (Edge + N-before-M modes)
        if (d->phaseCorrRemaining > 0) {
            d->phase -= d->phaseCorrPerSample;
            d->phaseCorrRemaining--;
            // Keep phase in range
