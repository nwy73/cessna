...
static void accumulateBin(struct _CessnaAwg_DTC *d, float x);
static void finalizeCycle(struct _CessnaAwg_DTC *d);
...
static bool processCycleDetection(struct _CessnaAwg_DTC *d, float x) {
    ...
    if (d->wasBelowNeg && (y > d->thresh)) {
        ...
        if (period >= d->minPeriodSamps && period <= d->maxPeriodSamps) {
            ...
            // Update phase increment for resynth oscillator
            if (d->smoothPeriod < 4.0f) d->smoothPeriod = 4.0f;
            d->phaseInc = 1.0f / d->smoothPeriod;

            // Simple lock heuristic: close to smoothed estimate
            float relErr = fabsf((float)period - d->smoothPeriod) / d->smoothPeriod;
            d->locked = (relErr < 0.02f);
...
// Accumulate input into 16 bins for the current cycle, based on an estimated cycle length.
static void accumulateBin(struct _CessnaAwg_DTC *d, float x) {
    // Estimate phase within the current input cycle using the smoothed period
    float ph = (d->smoothPeriod > 1.0f) ? ((float)d->cycleSampleCount / d->smoothPeriod) : 0.0f;
    ph = clampf(ph, 0.0f, 0.999999f);
    ...
    // (Optional) keep display buffer roughly phase-normalised
    int di = (int)floorf(ph * (float)(kDisplayPoints - 1));
    if (di < 0) di = 0;
    if (di >= kDisplayPoints) di = kDisplayPoints - 1;
    d->dispIn[di] = x;
}

// Finalize a captured cycle: compute per-bin means, update A with servo alpha (unless frozen),
// handle capture->freeze, and reset accumulators for next cycle.
static void finalizeCycle(struct _CessnaAwg_DTC *d) {
    ...
    // Update stored bins unless frozen
    if (!d->freeze) {
    ...
    // Capture: if armed and locked, freeze after this completed cycle
