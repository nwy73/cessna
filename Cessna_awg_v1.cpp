...
struct _CessnaAwg_DTC {
    ...
    float declickGain;      // 0..1, ramps up after a hard jump to mask phase-reset click
    float declickInc;       // per-sample increment for declickGain
    ...
    _CessnaAwg_DTC() {
        ...
        hardJump    = false;
        declickGain = 1.0f;
        declickInc  = 1.0f / (kDefaultSampleRate * 0.002f);  // 2ms ramp
...
static _NT_algorithm* construct(const _NT_algorithmMemoryPtrs& ptrs,
    ...
    d->declickInc   = 1.0f / (sr * 0.002f);  // 2ms ramp at actual sample rate
...
static bool processCycleDetection(struct _CessnaAwg_DTC *d, float x) {
    ...
    if (d->wasBelowNeg && (x > thresh) && (d->samplesSinceEdge >= d->minPeriodSamps)) {
        ...
        if (period <= d->maxPeriodSamps) {
            ...
            if (rel > 0.10f) {
                // Hard pitch jump — snap immediately and skip finalizeCycle
                d->smoothPeriod = (float)period;
                d->phase        = 0.0f;
                d->stableCount  = 0;
                d->locked       = false;
                d->hardJump     = true;
                d->declickGain  = 0.0f;  // trigger declick ramp
            } else {
...
static void step(_NT_algorithm *base, float *busFrames, int numFramesBy4) {
    ...
    for (int n=0;n<numFrames;n++) {
        ...
        // Declick: ramp gain back up after a hard pitch jump
        if (d->declickGain < 1.0f) {
            d->declickGain += d->declickInc;
            if (d->declickGain > 1.0f) d->declickGain = 1.0f;
        }
        y *= d->declickGain;
