...
static constexpr int kNumParams = (kParamSyncRatio + 1);
...
static const _NT_parameter g_parameters[kNumParams] = {
    ...
    { .name="Phase Lock",  .min=0,   .max=100, .def=50,  .unit=kNT_unitPercent, .scaling=kNT_scalingNone, .enumStrings=nullptr },
...
static void updateModGrayOut(_NT_algorithm* base) {
    ...
    // Read from DTC fields (always in sync) rather than self->v (may lag on load)
    int mode     = d->modMode;
...
// Uses weighted phase correction (scale lockStrength by waveform slope at current
// phase position — steeper slope = more confident correction, per Arora & Sethares).
static inline void applySync(_CessnaAwg_DTC *d, float newPeriod) {
    // Weighted phase correction
    if (!d->hardJump) {
        // Compute waveform slope at current phase position
        float phF  = d->phase * (float)kBins;
        int   bin0 = ((int)phF) & (kBins - 1);
        int   bin1 = (bin0 + 1) & (kBins - 1);
        float slope = fabsf(d->S[bin1] - d->S[bin0]) * (float)kBins;
        // Normalise: max slope for a ±5V signal over 1 bin ≈ 10*16 = 160
        float weight = clampf(slope / 160.0f, 0.0f, 1.0f);
        float effectiveLock = d->lockStrength * weight;
        d->phase *= (1.0f - effectiveLock);
    }
    // Frequency tracking with n:m ratio scaling
    if (d->trackStrength > 0.0f) {
        // Scale target frequency by P/Q ratio
...
static void step(_NT_algorithm *base, float *busFrames, int numFramesBy4) {
    ...
    for (int n=0;n<numFrames;n++) {
        ...
        // Gradient frequency accumulation: accumulate input * waveform slope
        // This implements the Arora & Sethares gradient frequency update.
        // Only accumulate when input is present and signal is above noise gate.
        if (d->env > 0.005f) {
            float phF2  = d->phase * (float)kBins;
            int   gb0   = ((int)phF2) & (kBins - 1);
            int   gb1   = (gb0 + 1) & (kBins - 1);
            float gSlope = (d->S[gb1] - d->S[gb0]) * (float)kBins; // signed slope
            d->gradAccum += x * gSlope;
            d->gradSamples++;
        }
        ...
        if (edge) {
            ...
            // Gradient frequency correction (Arora & Sethares eq. 11):
            // Nudge phaseIncTarget based on accumulated input*slope correlation.
            // Normalise by cycle energy (gradSamples) to keep gain stable.
            if (d->gradSamples > 0 && d->trackStrength > 0.0f) {
                float mu = 0.001f * d->trackStrength; // small stepsize per paper
                float gradNorm = d->gradAccum / (float)d->gradSamples;
                d->phaseIncTarget += mu * gradNorm;
                // Clamp to ±50% of current rate to prevent runaway
                float maxInc = d->phaseInc * 1.5f;
                float minInc = d->phaseInc * 0.5f;
                if (d->phaseIncTarget > maxInc) d->phaseIncTarget = maxInc;
                if (d->phaseIncTarget < minInc) d->phaseIncTarget = minInc;
            }
            d->gradAccum   = 0.0f;
            d->gradSamples = 0;
